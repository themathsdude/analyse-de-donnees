...
bwplot(Team_1986~Salary_1987, data = baseball)
```

On observe que l'équipe d'appartenance est discriminante pour le salaire. 
En particulier, il est évident qu'une anova nous donnera une influence de 
l'équipe sur le salaire. Même si les équipes contiennent moins de 30 joueurs
...
```{r corrélations-années, results = 'hide', out.width = "80%"}
...
De même, ici on peut rejetter les hypothèses sur les coefficients de régression nuls.
Pour l'effet cumulé de Runs et Hits, on peut rejetter la nécessité de garder ce 
...
```{r}
acf(reg.multiple.86$residuals)
```

A partir du correlogramme nous confirmons l'absence d'autocorrelation des résidus. 
...
Anova nous permets de confirmer notre remarque précedentes.

```{r anova-res-runs}
summary(aov.res_runs)
```

Pour le home runs, la p value est supérieur à 5%, de ce fait on conclut le home runs n'a pas un grand impact sur les salaires.

### B - Tests de student

Nous utilisons le test de student comme alternatif de Anova sur les 2 groupes avec des salaires élevés. 

```{r test-student}
t.test(Hits_86~Salary_Group, data = subset(baseball_no_na, Salary_Group == 1 | Salary_Group == 2))
```

Ceci nous permets de confirmer le fait qu'il y a une difference significative entre les moyennes de hits_86 des deux groupes.
