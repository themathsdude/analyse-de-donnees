...
histogram(baseball$Salary_1987,horizontal=TRUE)
baseball_no_na$Log_Salary_1987  <- log(baseball_no_na$Salary_1987)
Log_salary <- baseball_no_na$Log_Salary_1987
histogram(baseball_no_na$Log_Salary_1987,horizontal=TRUE)
...
boxplot_longevite
```
Nous constatons l'éxistence de trois phases d'évolution de salaires par rapport aux années expériences professionnelles:

- Phase 1: [1,5] c'est le début de carrières des joueurs de Baseball avec des salaires faibles mais qui augmentent avec le temps. l'emergence de nouvelles stars avec des salaires remarquables.  
- Phase 2: [6,13] c'est la phase de la maturité professionnelles où des joueurs se différencient par rapport à la mediane, c'est l'age d'or des joeurs ils ont touché des salaires importants.
- Phase 3: [14,24] c'est la fin des carrières, nous observons que des joueurs démissionnent à partirs de l'année 18, les salaires en parallèles diminuent. Des cas de figures exceptionnelles restent toujours sur le marché et réussissent à garder leur salaires interessants.
...
Runs_career/Longevity ~ Log_Salary_1987




```{r regression, echo=TRUE}
reg.res <- lm(Runs_career/Longevity ~ Log_Salary_1987, data = baseball_no_na)
summary(reg.res)
```
...
```{r regression, echo=TRUE}
reg.res1 <- lm((Runs_career - Home_runs_career + Hits_career)/Longevity ~ Log_Salary_1987, data = baseball_no_na)
summary(reg.res1)
```
```{r}
plot(reg.res1)
```
```{r}
stdz <- (reg.res1$residuals - mean(reg.res1$residuals))/sd(reg.res1$residuals)
histogram(stdz)
histogram(reg.res1$residuals)
